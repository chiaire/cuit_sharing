...
public class aes{
    ...
    public static void main(String[] args) throws Exception{
        ...
        //String转化成10进制
        int[][] roundKey_24 = new int[4][4];
        for(int i=0;i<b.length;i++){
            for(int j=0;j<b.length;j++){
                Integer integer = Integer.parseInt(b[j][i],16);
                //System.out.println(integer);
                roundKey_24[j][i] =integer;
            }
        }

        //轮密钥加密函数调用
        Integer[][] roundKeyAddedState_24 = AddRoundKey_24(state_24,roundKey_24);

        String[][] roundKeyAddedStateHex_24 = new String[4][4];
        for(int i = 0;i<4;i++){
            for(int j=0;j<4;j++){
                String string = Hex_24_24(roundKeyAddedState_24[i][j]);
                //System.out.println(string);
                roundKeyAddedStateHex_24[j][i] = string.toUpperCase();
            }
        }
        ...
        for(int i =0;i<4;i++){
            for(int j = 0;j<4;j++){
                System.out.print(roundKeyAddedStateHex_24[i][j]+" ");
            }
            System.out.println();
        }

        //字节替代调用
        String[][] string = SubBytes_24(roundKeyAddedState_24);

        //行移位函数调用
        String[][] strings = ShiftRows_24(string);

        //列混合函数调用
        MixColumns_240(strings);

    }


    //轮密钥加密函数
    private static Integer[][] AddRoundKey_24(int[][] p1, int[][] p2) {
    ...
    //字节替代函数
    private static String[][] SubBytes_24(Integer[][] p1){
    ...
    //行移位输出
    private static String[][] ShiftRows_24(String[][] p1){
        String s,s1,s2;
    ...
    //列混合函数
    //固定矩阵输入
    private static void MixColumns_240(String[][] p1){
        ...
        for(int i=0;i<4;i++){
            for(int j=0;j<4;j++){
                System.out.print(Hex_24_24(integers3[i][j]).toUpperCase()+" ");
            }
            System.out.println();
        }
    }
